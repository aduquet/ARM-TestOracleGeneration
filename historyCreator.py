...
def saveFile(df, file_out, name, ti):
    df.to_csv(file_out + name)
    print('\n *** Historical Data is saved in *** \n', file_out + name)
    t = open(file_out + name + "_timeReport.txt", 'w')
    t.write(str(datetime.datetime.now() - ti))
    t.close()
    print('done!')
    ...
    def main(file_in, d, file, file_out):
        ...
        if d == 'dir':
            ...
            for i in range(0, len(dataPath)):
                ...
                df.drop(df[df['testId'] == '--'].index, inplace=True)
                ...
                new_df = histoData(df)
                # print(file_out)
                saveFile(new_df, file_out_aux, name, begin_time)
            ...
            saveFile(new_df, file_out, name='.csv', ti=begin_time)
            print('done!')
