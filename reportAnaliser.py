...
if __name__ == '__main__':
    ...
    @click.command()
    @click.option('-i', '--file', 'file_in', help='Path for getting the data full')
    ...
    def main(file_in, direc, file_out):
        ...
        if direc == 'directory':
            ...
            for i in range(0, len(dataPath)):
                data = pd.read_csv(dataPath[i], index_col=0)
                name = dataPath[i].split('\\')
                name = name[-1]

                df= data['diff'].value_counts()
                ...
                #print('YES',df_yes, df_no)
                #print(df)
               # print(df[0])
                #print(df[1])

                data2 = {'DS': name, '# of Total rows': len(data),
                         '# of rows that are NOT different from Non-mod (correct values)': df_no,
                         '# of rows that are different from Non-mod (incorrect values)': df_yes}
